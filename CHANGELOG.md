...
- **Per-tenant issuance UI policy, published as configuration**
  ...
  - **Loaded at bootstrap, resolved once.** `IssuanceUiPolicyService.load()` runs in `main.ts`'s `APP_INITIALIZER` after `TenantService.resolve()` (it needs `tenant()`) and in parallel with the theme, so it adds no round trip. The initializer blocks the first paint and the metadata it filters is fetched later still, so the type selector can never render against an unloaded policy. There is no background retry re-applying a late answer — a selector that changes under the user's pointer is worse than a deterministic one (EUD-217 AD-4/EC-03).
