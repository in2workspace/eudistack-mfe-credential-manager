...
- **EUD-226 — Gestionar el contacto de la organización para notificaciones del ciclo de vida (US-07)**
  ...
  - **Authorization guard (AC-03, AC-04)**: `organizationContactGuard` and `NavbarComponent.canSeeOrganizationContact` now read real capability from `AuthService` (`organizationContactEnabled` sourced from `GET /api/v1/me`, `canWriteOrganizationContact()` derived from `roleType()`) — the menu entry and route are correctly hidden/denied for a disabled tenant flag or a Caso A (multi-org, read-only) role, both fail-closed on init/error.
  ...
  - **Tests**: Service, component, guard, and navbar unit tests, all passing under Jest (82/82 suites, 1063/1063 tests, 88.09% coverage) — including 6 scenarios exercising the real guard/navbar capability logic (feature-disabled deny, Caso A deny, Caso B/C allow).
  - **Known limitation, out of this Story's scope**: `OrganizationContactComponent`'s `orgId` is not yet wired to the real authenticated context (`placeholder-org-id`) — tracked separately, does not affect the guard/authorization fixes above.
