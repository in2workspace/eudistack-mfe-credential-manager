...
- **EUD-226 — Gestionar el contacto de la organización para notificaciones del ciclo de vida (US-07)**
  ...
  - **API integration (FR-17)**: `OrganizationContactService` with `fetchContact(orgId): Observable<OrganizationContact>` (GET `/api/v1/organizations/{id}/contact`) and `updateContact(orgId, email): Observable<void>` (PUT `/api/v1/organizations/{id}/contact`). Handles 400/403/404/5xx errors and timeouts (ES-04, ES-05).
  - **Authorization guard (AC-03, AC-04) — KNOWN GAP, NOT YET ENFORCED**: `organizationContactGuard` and `NavbarComponent.canSeeOrganizationContact` are currently stubs — both `featureEnabled`/`canWrite` are hardcoded `true`, so the menu entry is always visible and the route always reachable regardless of tenant flag or role. See `/code-review` `quality-report.md` finding B3/F4 (HIGH). Two of the test files for this slice (`organization-contact.component.spec.ts`, `organization-contact.guard.spec.ts`) also fail to compile under Jest (Jasmine syntax used by mistake — finding B4) and contribute zero coverage.
  - **Model**: `OrganizationContact { email: string | null }` interface in `core/models/entity/organization-contact.ts`.
  - **i18n (AC-01, AC-02, EC-01, ES-01)**: Added keys in `es.json`, `en.json`, `fi.json` for page title, email label, save button, success/error messages, and menu entry. Finnish (`fi.json`) translations for PRH Finland tenant.
  - **Tests**: Service unit tests (`organization-contact.service.spec.ts`, 7 scenarios, passing). Component and guard test files exist but fail to compile (see gap above) — zero real coverage for AC-01, AC-02, EC-01, ES-01, ES-04, ES-05 (component) and AC-03, AC-04, EC-04 (guard). Navbar tests (`navbar.component.spec.ts`, 1 active + 2 `xit` TODOs for menu entry visibility) compile and pass but the 2 `xit` cases covering this Story's ACs are skipped.
  - **Status:** `/code-review EUD-226` (2026-08-17) returned CHANGES REQUESTED with 5 BLOCKING findings (backend + frontend). Feature flag remains `false` by default for all tenants pending fixes. See `docs/EUD-5-gestion-ciclo-vida-portal/EUD-226/quality-report.md`.
