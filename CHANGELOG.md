...
- **EUD-72 — Credential catalogue settings screen (US-02)**
  - **Technical debt:** : The link to 'Settings' in the `NavbarComponent` and the 'Catalogue' link in the `SettingsComponent` have been temporarily disabled while the `/admin` backend route is not available (the infrastructure doesn't redirect requests with '/admin' to the backend).
  ...
  - **EC-01 — empty selection is refused, not sent.** Saving with every toggle off would send an empty set, which makes the backend drop the tenant configuration and re-enable *all* types — the opposite of what switching everything off looks like. `canSave()` now blocks it and an `role="alert"` warning next to the button explains why (previously the empty set was allowed with an informational note). Emptiness is measured on the full registry, not on the visible rows: a superseded version left enabled keeps the payload non-empty, so warning on the visible rows would block a save that is safe.
  ...
  - **i18n.** `catalog.*` and `sidenav.catalog` added in `es`, `en` and `ca`, later extended with `catalog.version`, `catalog.error.not-configured.*` and a shared `unsavedChanges.leaveAlert`; `catalog.description`, `catalog.empty-set-warning` and `catalog.toggle-aria` reworded to match the blocked-empty-save behaviour and the format/version row.
