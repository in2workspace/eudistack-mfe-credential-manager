...
### Security
- **EUD-233 — `/code-review` hardening (2026-09-17): dos hallazgos de `security-auditor` en el ciclo de vida del par de claves, sin cambio de comportamiento observable.** (1) `HolderPrivateKeyStore` se limpia ahora también al **inicio** de cada envío (antes de generar) y al cerrar cualquiera de las dos superficies post-emisión, cerrando los dos únicos puntos de los seis de AD-6 que faltaban por implementar (los otros cuatro ya estaban) — sin ruta alcanzable hoy que dejara la clave residente más allá de su intento, pero la garantía pasa de accidente de control de flujo a control explícito. (2) El fallo de generación del par (`HolderKeyGenerationError`, ES-09) ya no se registra dos veces: su propio rastro de cuatro campos cerrados (AD-15) seguía siendo correcto, pero el manejador genérico de fallos de envío lo volvía a loguear con el objeto completo (`cause`/stack incluidos) y un mensaje incorrecto ("POST failed" cuando no se llegó a enviar nada) — ahora ese manejador distingue este caso y solo emite los campos ya permitidos.
- **EUD-233 — `/code-review` hardening (2026-09-17): la mitad pública del par de claves (`HolderKeyStoreService`) pasa a ir sellada, igual que la privada.** Hasta ahora `HolderPrivateKeyStore` verificaba que la clave recuperada perteneciera exactamente al intento de emisión en curso (`credentialConfigurationId` + `submissionId`), pero su mitad pública (la que viaja como `holder_key.jwk` y vincula el `cnf` de la credencial) se leía sin esa verificación — asimetría señalada por auditoría de seguridad, sin ruta de explotación conocida hoy (el flujo es síncrono y los envíos no se solapan), pero cerrada por consistencia: `HolderKeyStoreService` gana la misma verificación de sello que ya usaba la clave privada.
- **EUD-233 — `/code-review` hardening (2026-09-17): la credencial firmada copiada al portapapeles gana el mismo TTL de auto-borrado (≤ 60 s) que la clave privada ya tenía.** La credencial de `learcredential.machine` porta datos personales del mandador (nombre, email, número de serie, identificador de organización) y quedaba en el portapapeles del sistema indefinidamente tras copiarla, a diferencia de la clave privada — auditoría de seguridad lo señaló como inconsistente con la práctica ya aplicada a la clave. Mismo mecanismo ya existente (`CopyableFieldComponent`), sin diálogo ni control nuevo.

### Fixed
- **EUD-233 — el texto de ayuda del QR ("Escanea este código QR...") se renderizaba fuera de la caja del QR en la vista de resultado por canal.** Movido al propio `CredentialOfferQrComponent`, justo antes de la imagen del QR, así queda siempre dentro de su caja sin importar qué host lo embeba (`CredentialOfferDialogComponent` en solitario o anidado dentro de `DeliveryOutcomeListComponent`).
- **EUD-233 — el aviso "Copia la credencial firmada antes de cerrar..." de `DirectCredentialResultDialogComponent` no nombraba la clave privada cuando la superficie también la presentaba.** El texto ahora se deriva de qué artefactos presenta esta superficie (credencial sola, o credencial + clave), fijado al abrirse — a diferencia de un intento anterior en esta misma corrección, deliberadamente **no** se deriva de qué queda aún sin copiar: copiar la credencial primero no debe hacer que el aviso deje de mencionarla.
- **EUD-233 — post-release PO fix: cerrar el modal de resultado tras un envío directo dejaba al operador de vuelta en el formulario reseteado en vez de navegar al listado.** `UncopiedArtifactCloseGuard` llamaba `history.back()` en **todo** cierre terminal para retirar el centinela de historial que empuja al abrir la superficie protegida (`pushSentinel()`), incluido un cierre normal por "Done" — pero `history.back()` es una navegación real y asíncrona del navegador. Podía resolverse **después** de que el `router.navigate(['/organization/credentials'])` del propio host ya hubiera empujado la nueva URL, deshaciendo ese push y dejando al operador en la entrada del centinela (la misma URL del formulario, ahora en blanco). El fallo alcanzaba a cualquier superficie protegida cerrada por una vía distinta del botón "atrás" real del navegador (Done, backdrop, Esc, control secundario), no solo a tipos de máquina — el reporte inicial simplemente los probó primero. `history.back()` ahora se reserva al único caso en que sí hay una navegación de verdad que completar: un descarte confirmado tras un `popstate` real que el guard había neutralizado; el resto de cierres retiran el centinela sin invocar ninguna API de navegación, eliminando la carrera de raíz. Sin distinción por tipo de credencial: la superficie post-emisión y su guarda son las mismas para todos.
- **EUD-233 — post-release PO fix: el banner de "catálogo de entrega ilegible" (ES-08) parpadeaba brevemente en cada carga del formulario, incluso cuando la lectura del catálogo iba a tener éxito.** `hasDeliveryCatalogReadFailed$` se derivaba solo del estado de la lectura, sin considerar que esta arranca en un valor de "no legible" por defecto mientras las tres cargas iniciales (política de UI, metadata, elegibilidad de entrega) siguen en vuelo — ahora queda condicionado también a `!isLoadingCatalog$()`, así que la única vez que se muestra es tras una lectura que de verdad ha fallado.
...
### Changed
- **`serialNumber` is no longer a required field in the issuance form.**
  - **Where the requiredness came from — two independent sources, both changed.** (1) `serialNumberValidatorEntries` in `shared/validators/credential-issuance/validators-entries.ts` carried `{ name: 'required' }`, and it is the only source of the validators on `serialNumberField` (`common-issuance-schema-fields.ts`), which both `LearCredentialEmployeeSchemaProvider` and `LearCredentialMachineSchemaProvider` place in their `mandator` group. (2) `PROVISIONAL_REQUIRED_KEYS` in `field-validation-rule.resolver.ts` listed `serialNumber`, so a `serialNumber` claim derived from `credential_metadata.claims` on the AD-2 `mandatee` path picked up a `required` validator as well — even though `ClaimDefinitionDto` declares no required flag and the resolver's own doc describes the set as provisional, mirroring the legacy validator entries. Changing only one of the two would have left the other still blocking.
  - **Why it blocked issuance.** The `mandator` group is `display: 'pref_side'`: `IssuanceSchemaBuilder.shouldExtractStatic()` turns it into read-only side data when `onBehalf === false`, but keeps it as real form controls when `onBehalf === true` (a SysAdmin issuing for another organisation). Those controls are seeded from `AuthService.extractRawMandator()`, which resolves `serialNumber` to `''` whenever the session's certificate carries no `serial_number`. An empty required control leaves the entire form invalid and the submit button disabled, with the only error message rendered in a side block the Operator has no reason to look at. The serial number is not needed to identify the mandator either: `IssuanceRequestFactoryService` builds `mandator.id` as `did:elsi:` over country + `organizationIdentifier`, and carries `serialNumber` through untouched.
  - **The shape rules stay.** `minLength 7`, `maxLength 15` and `^[a-zA-Z0-9-]+$` remain on the field. Angular's `minLength`/`maxLength`/`pattern` all pass on an empty value, so they now apply only once something is typed — a serial number that *is* filled in is validated exactly as before. Only the "must be filled in at all" rule is gone.
  - **Payload note.** An untouched control submits `serialNumber: ""` — the form control default, not an absent key — and `IssuanceLEARCredentialEmployeePayload` / `IssuanceLEARCredentialMachinePayload` still type it as `string`, so nothing downstream changes shape. If the Issuer ever rejects an empty string here, the fix belongs in the payload mapping (omit the key when blank), not in restoring the validator.
  - **Tests.** `field-validation-rule.resolver.spec.ts` gains a case asserting `serialNumber` resolves to `required: false`. The AC-06/AC-07 regression guard in `lear-credential-employee-issuance-schema-provider.spec.ts` used `serialNumber` as its example of "a key in the provisional required set" and now uses `organization`, which is likewise absent from `PROVISIONAL_MANDATEE_FIELDS` and therefore still exercises the resolver rather than the field overrides; a new sibling case asserts a derived `serialNumber` claim gets no `required`.
...
### Changed
- **EUD-233 — `/code-review` hardening (2026-09-17): `UncopiedArtifactCloseGuard.protect()` exige ahora declarar explícitamente que el diálogo se abrió con `closeOnNavigation: false`.** `MatDialogRef` no expone la configuración con la que se abrió, así que la guarda no podía verificarlo por sí misma (R-15: sin ese flag, Material cierra el diálogo al navegar antes de que la guarda pueda reaccionar al botón "atrás"). Auditoría de seguridad señaló que un futuro tercer diálogo protegido podría omitir el flag sin que nada lo detectara. `protect()` ahora recibe `{ closeOnNavigationDisabled: boolean }` y lanza un error inmediato si es `false`, en vez de fallar en silencio la primera vez que alguien lo olvide. Los dos diálogos actuales ya declaraban el flag correctamente — sin cambio de comportamiento para ninguno de los dos.
- **EUD-233 — post-release PO polish: el desglose de resultado por canal (`DeliveryOutcomeListComponent`) pasa de lista plana a cajas con borde por modo.** Sustituida la `<ul>` de una línea por icono/color de texto por una caja `<section>` bordeada por cada modo solicitado — borde y texto verdes (`--status-success`) si entregó, rojos (`--status-error`) si falló, nunca el gris neutro — con un título (la misma `labelKey` que ya usan las casillas de entrega) y un texto propio por combinación modo/resultado (p. ej. "Credencial entregada." vs "No se ha podido emitir la credencial, así que no se devuelve aquí." para `direct`). Inspirado en el layout de `feat/direct-delivery` pero sin adoptar su estructura de un único diálogo "handover": la copia/clave de esta Story sigue en sus propios bloques (`app-copyable-field`/`app-holder-private-key-section`) fuera de este componente, y el QR de oferta sigue siendo hermano suyo, no anidado dentro. La visibilidad (`showOutcomes` en cada host) no cambia — AC-04/EC-07 y AC-05.2/EC-09.1 se conservan intactos, ver `functional-design.md` §1.3.
...
- **EUD-233 — post-release PO polish: las casillas de entrega usan el color de marca del tenant, igual que los radio buttons.** `default-theme.scss` ya sobreescribía las variables MDC de los radio buttons (`--mdc-radio-selected-*-icon-color: var(--primary-color)`) pero nunca las equivalentes de `mat-checkbox`, así que las tres casillas del selector de entrega se quedaban con el azul neutro de Material. Añadido el bloque análogo para `.mat-mdc-checkbox .mdc-checkbox` (`--mdc-checkbox-selected-*-icon-color` + `--mdc-checkbox-selected-checkmark-color`), aplicado globalmente a cualquier `mat-checkbox` de la app.
- **EUD-233 — post-release PO polish: en la confirmación de "cerrar sin copiar" (AC-14), el botón destacado pasa a ser "Seguir aquí", no "Cerrar igualmente".** `BaseDialogData` gana `emphasizeCancel?: boolean` (opt-in, `undefined`/`false` mantiene el comportamiento de hoy byte a byte): cuando es `true`, `DialogComponent` intercambia qué botón recibe `statusColor` frente a `color="cancel"` — los `(click)` siguen ligados a `onCancel()`/`onConfirm()` sin cambios, solo se mueve el peso visual. `UncopiedArtifactCloseGuard.confirmDiscard()` es el único emisor que lo activa; el resto de diálogos que comparten `DialogComponent` no se ven afectados.
...
- **EUD-233 — superficie de resultado solo-Wallet con apartado de clave privada (AC-13) y el modal `direct` de un solo uso (AC-01/AC-07).** Cuando `direct` se entrega, `DirectCredentialResultDialogComponent` (nuevo) muestra la credencial firmada y, si el tipo lo exige, la clave privada, cada una en su propio campo copiable con TTL de borrado de portapapeles independiente (60 s para la clave, sin TTL para la credencial). Cuando `direct` no se declaró pero el tipo exige la clave, `CredentialOfferDialogComponent` (AS-IS desde EUD-167) se extiende en el mismo sitio con un apartado de clave por encima de su contenido Wallet, nunca como una pantalla aparte — ausencia de clave en el cliente (p. ej. recarga entre el envío y la respuesta) se resuelve con un aviso no bloqueante (AC-10.1/AC-10.2), no con un error.
- **EUD-233 — guarda de cierre sobre artefactos sin copiar (AC-14, `UncopiedArtifactCloseGuard`, nuevo, `providedIn: 'root'`).** Backdrop, Esc, el control de cierre secundario y el botón "atrás" del navegador quedan interceptados mientras quede un artefacto (`'credential' | 'privateKey'`) sin copiar, y desembocan en el diálogo de confirmación ya existente (`DialogWrapperService`, `confirmationType: 'sync'`) — nunca un diálogo nuevo. Cancelar devuelve a la superficie original intacta; confirmar cierra de verdad. Sin artefactos pendientes, cualquiera de las cuatro vías cierra directo, sin preguntar.
- **EUD-233 — lectura simétrica del catálogo del tenant, con los cuatro estados de elegibilidad ya vistos en EUD-169 aplicados también al lado de emisión.** `resolveDeliveryEligibility` distingue catálogo resuelto y abierto, campo `delivery_modes` ausente (tenant pre-EUD-169, retrocompatible), array vacío (tipo sin ningún modo elegible — se retira del selector) y catálogo ilegible, que ahora muestra un `AlertBannerComponent` (nuevo) no bloqueante en vez de fallar en silencio o bloquear el formulario.

### Changed
- **EUD-169 — el path del catálogo de credenciales deja de sugerir "solo admin".** `API_PATH.CREDENTIAL_CATALOG` pasa de `/admin/v1/credential-catalog` a `/api/v1/backoffice/credential-catalog` (`eudistack-core-issuer`, mismo backend, mismo contrato — `GET`/`PUT` sin cambios de forma ni de autorización). Renombre puro de path: el `code-review` de re-verificación de EUD-169 (hallazgo H1) señaló que `/admin` ya no describía el endpoint desde que su `GET` se abrió también al rol operador (`LEAR`) de la Story; `/api/v1/backoffice/...` recupera además la convención que ya usaba el módulo predecesor retirado (`delivery-config`).

### Removed
- **EUD-233 — `KeyGeneratorComponent` y su formulario incrustado.** Sustituido por la generación invisible del par dentro del comando de envío (`IssuanceHolderKeyService`, ver `Added` arriba): el operador ya no maneja la clave a mano en ningún tipo de máquina, la ve (si aplica) en la superficie de resultado tras el envío.

### Fixed
- **EUD-168 — code-review (PR): la JWK del holder se drenaba antes de confirmar el éxito real, y un canal fallido en un 207 contaba como envío correcto.** `withHolderKey()` leía `HolderKeyStoreService` con `take()` (destructivo) antes incluso de lanzar el POST: un fallo HTTP (400/403/5xx/timeout) dejaba el store vacío sin que `handleIssuanceFailure` lo restaurase, así que un reintento sin regenerar la clave llegaba sin `holder_key` — y si el operador sí regeneraba, la máquina quedaba con la privada de un par distinto al que finalmente se vincula la credencial. Separadamente, `hasSubmitted$.set(true)` corría para cualquier 2xx antes de comprobar `hasChannelError`, incluido un 207 con `responses[].error`: `canLeave()` quedaba `true` aunque la creación hubiera fallado parcialmente. `HolderKeyStoreService` gana `peek()` (lectura no destructiva) en lugar de `take()` (retirado); `submitCredentialPayload()` solo marca `hasSubmitted$` y vacía el store dentro de la rama que confirma éxito real, tras descartar `hasChannelError`. Un fallo HTTP o un 207 con canal fallido dejan ahora la misma clave lista para el reintento, sin necesidad de regenerarla. Tests nuevos: retry tras fallo HTTP conservando `holder_key`, retry tras 207 conservando la clave, y `hasSubmitted$() === false` en ambos casos.
- **EUD-168 — code-review: `generateKeys()` podía escribir en un formulario y un store ya destruidos.** Tras el `await generateP256()` no había guarda de destrucción: un Generate → Cancel mientras WebCrypto aún resolvía dejaba que la continuación llamase `patchValue()`/`holderKeyStore.set()` sobre un componente ya destruido, reabriendo -- en una ventana de carrera estrecha -- lo que `ngOnDestroy` ya cerraba (TD-04 en `tech-debt.md`): una JWK pública de un par cuya privada nadie llegó a ver quedando en el store (`providedIn: 'root'`, sobrevive al componente) para una emisión posterior. `KeyGeneratorComponent` gana un flag `destroyed`, marcado al inicio de `ngOnDestroy`, comprobado tras el `await` antes de tocar el form o el store.

## [3.7.0] - 2026-09-07

### Fixed
- **BREAKING — issuance response reading adapted to the backend's new `responses[]` envelope (EUD-167 D-5/D-6).** `eudistack-core-issuer` replaced its flat `signed_credential`/`credential_offer_uri` issuance response fields with a nested `responses[]` array (commit `4ab3600b`); the old top-level `credential_offer_uri` field this app read no longer exists on the wire, so this was required to keep the QR-offer dialog working, not an optional enhancement. `IssuanceResponseDto` gains `IssuanceChannel`/`IssuanceChannelBody`/`IssuanceChannelError`/`IssuanceChannelResponse`. `CredentialIssuanceService.submitCredentialPayload()` now reads the offer URI out of `responses[].body.credential_offer_uri` (same semantics as before: only the `ui` channel ever carries one) and, defensively, treats any channel-level `error` as a failure even on a 2xx/207 response — Angular's `HttpClient` never routes a `207 Multi-Status` to `catchError`, so a mixed outcome has to be read from the body on the success path. Not yet reachable from the form (one delivery mode per request today), but closes the gap `docs/_shared/reference/tech-debt-register.md` `TDG-1` (`eudistack-platform-dev`) flagged: a future multi-mode submission must not silently render a partial failure as success. *(Code-review N2, same day: reconciled a stray comment that contradicted this — no functional change, the `.find()` over all channels already handled `email`'s absent `body` correctly.)*

- **EUD-168 — `displayedKeys$` didn't type-check against the machine-credential key state, and hid an already-generated private key once a sibling field was set on its own.** `KeyGeneratorService.displayedKeys$` was typed `Signal<Record<string, string> | undefined>`, incompatible with `KeyState.desmosPublicJwk` (a structured JWK, not a string) once that field was added — a `TS2322` that failed `key-generator.component.spec.ts` to compile. New `DisplayedKeyState` narrows the signal to just the text fields the template renders, keeping the JWK out of it entirely. Separately, the computed's truthiness check on the private-key string hid it once `desmosDidKeyValue` was set without a private key alongside it (an empty string is falsy) — it now always returns the current value, whatever it is; what's worth rendering is the template's own per-row check, not this signal's.

### Changed

- **EUD-168 — la credencial de máquina envía su `holder_key` desde el formulario**: los tipos `learcredential.machine.*` toman su `cnf` del cuerpo de la petición de emisión, en **todos** los modos de entrega (excepción acotada AD-8): retirado `proof_types_supported` de su esquema, ningún key proof llega tampoco por el flujo de Wallet. Sin este cambio esos tipos dejaban de poder emitirse desde el portal — el Issuer respondía `400 invalid_holder_key` incluso en modo `ui`.
  - El formulario **ya generaba** el par P-256 (`KeyGeneratorComponent`, incrustado como campo del esquema de máquina) y ya mostraba la clave privada al operador. Lo que faltaba era la mitad pública: `KeyState` gana `desmosPublicJwk` y `generateP256()` la exporta del mismo `CryptoKeyPair`, de modo que las tres representaciones no pueden discrepar.
  - `HolderKeyStoreService` (root) traslada **solo la mitad pública** del widget a la petición. `KeyGeneratorService` sigue provisto en el componente a propósito —su estado muere con el formulario, que es el ciclo de vida correcto para una clave privada— y el componente no puede inyectar `CredentialIssuanceService` sin cerrar un ciclo de imports. El store se vacía al leerse: una clave olvidada ahí vincularía la siguiente credencial emitida a la clave de la anterior, sin que nada lo revelara.
  - La exención se reconoce por **familia** (`learcredential.machine.sd.`, `learcredential.machine.w3c.`), igual que en el backend, así que un cambio de versión no exige tocar código en ninguno de los dos repositorios. Nada valida en build que ambas listas coincidan: el test que fija los prefijos es la única defensa contra la divergencia.
  - **Ciclo de vida de la mitad privada (`/code-review`, cierra `tech-debt.md` TD-04)**: `KeyGeneratorComponent.ngOnDestroy` vacía ahora el estado de `KeyGeneratorService` (nuevo `clearState()`) y programa el borrado del portapapeles a los ≤60 s de cada copia (temporizador independiente del de feedback visual, reiniciado en cada copia en vez de acumularse). Una primera versión de este cierre **cancelaba** el borrado pendiente al destruir el componente en vez de ejecutarlo — el caso dominante (copiar la clave y cerrar el formulario para pegarla en la máquina) dejaba la clave en el portapapeles indefinidamente; ahora se ejecuta de inmediato. `ngOnDestroy` también vacía `HolderKeyStoreService`, para que un formulario abandonado sin llegar a enviarse no deje una JWK pública vinculable a la siguiente emisión.
...
### Fixed
- **El logout OIDC ya no deja al usuario en una pantalla inconsistente cuando el Verifier rechaza la solicitud.** Tras un `id_token_hint` caducado (p. ej. sesión inactiva largo tiempo), el Verifier ahora redirige a `<loginPageUri>?error=session_expired` en vez de exponer su JSON de error. `AuthService.checkAuth$()` detecta ese parámetro al aterrizar, limpia el estado de sesión local (`sessionStorage`, incluidos los artefactos PKCE/token de `angular-auth-oidc-client`) y muestra un diálogo guiando a reiniciar sesión — sin disparar el reintento silencioso de SSO, que habría vuelto a autenticar al usuario justo después de haber pedido cerrar sesión. Se extrajo `resetLocalAuthState()` a partir del bloque que antes solo vivía en el fallback de error de `logout()`, reutilizado ahora también aquí.
- **El botón de revocación vuelve a aparecer en las credenciales `dc+sd-jwt`.** `credentialStatusHasRevokeCredentialButton` era una lista blanca de `['BitstringStatusListEntry']`, añadida para ocultar la acción en las credenciales heredadas de DOME con `PlainListEntity`, que no tienen índice en ninguna lista de estado y hacen fallar el endpoint de revocación. Una credencial SD-JWT no lleva un `credentialStatus` de W3C: el emisor le escribe `status.status_list: { uri, idx }` (IETF Token Status List), que `LEARCredentialDataNormalizer` traduce al sobre unificado con `type: 'TokenStatusListEntry'` — fuera de la lista blanca, así que `showRevokeCredentialButton$` era `false` para toda credencial SD-JWT aunque estuviera `VALID`. El fallo quedó latente hasta que el emisor pasó a derivar el formato del perfil en lugar de la fila de emisión (EUD-215): antes, las filas con `credential_format` obsoleto bajaban por la rama W3C y la acción se veía. Ahora es una lista negra de `PlainListEntity` — la intención original —, de modo que cualquier mecanismo de estado futuro es revocable salvo prueba en contra. La revocación en sí no distingue formato: el emisor resuelve el índice por `issuanceId` sobre la misma tabla `status_list_index` para ambos. `CredentialStatusType` incluye ahora `'TokenStatusListEntry'`; el normalizador lo escribía sobre `any`, así que TypeScript nunca vio la incoherencia. Cubierto con el caso `TokenStatusListEntry` en los tests del helper y del servicio de detalles, y con el mapeo `status.status_list` → `credentialStatus` en los del normalizador, que no tenía cobertura.
...
- **`ServeErrorInterceptor` deja de abrir el diálogo de error bloqueante para el catálogo de credenciales.** Las respuestas de `issuer/admin/v1/credential-catalog` (cualquier código, incluido un `200` inesperado con HTML) se registran y se relanzan en silencio, igual que ya ocurría con los assets estáticos y el endpoint de IAM. La pantalla de catálogo (`CredentialCatalogComponent`, EUD-72) ya renderiza sus propios estados de error — forbidden, no-configurado, genérico con reintento — para cada forma de fallo de este endpoint; el diálogo global se superponía a esa pantalla y ocultaba su acción "Reintentar", convirtiendo un error recuperable en un callejón sin salida con solo "Cerrar". La detección de ruta (`isCredentialCatalogEndpoint`) respeta límites de segmento, así que una ruta que solo termine con un sufijo parecido (p. ej. `/issuer/api/v1/not-credential-catalog`) sigue mostrando el diálogo. Cubierto con casos para ruta relativa, absoluta y URL completa, y para el `200`-con-HTML que producía la ausencia de la ruta `/issuer/admin/*` en CloudFront — corregida en paralelo en `eudistack-platform-iac`.
