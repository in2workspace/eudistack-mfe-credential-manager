...
- **EUD-233 — el aviso "Copia la credencial firmada antes de cerrar..." de `DirectCredentialResultDialogComponent` seguía mencionando la credencial aunque ya estuviera copiada y solo quedara pendiente la clave privada.** El texto ahora se deriva de `pendingArtifacts` (AD-16): distingue los tres casos (solo credencial, credencial + clave, solo clave) en vez de un mensaje fijo.
