 ...
 * pair it produces -- `did:key` and the public JWK travel back to the caller as a `HolderBinding`,
 * the private half is sealed into `HolderPrivateKeyStore` under (`credentialConfigurationId`,
 * `submissionId`) -- clears `KeyGeneratorService`'s own state in the same tick (hardens
 * `NFR-S-EUD168-04(a)`, AD-6 cleanup point 5), and on failure leaves a diagnostic trace for
 * engineering only, before throwing a typed error the generic submission error path does not need
 * to inspect (ES-09, AD-15).
 *
...
export class IssuanceHolderKeyService {
  ...
  /**
   * Defense-in-depth teardown of both halves of the pair: `KeyGeneratorService`'s own state
   * (already cleared in the same tick by `generateForSubmission`, idempotent here) and the sealed
   * private-key handoff. The single entry point `CredentialIssuanceService` calls from its
   * `DestroyRef` hook (AD-6 cleanup point 6) -- the transport-failure, declared-and-failed, and
   * surface-close cleanup points (1-4) act on `HolderPrivateKeyStore` directly instead, per the
